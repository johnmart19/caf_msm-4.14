...
SECONDS=0
ZIPNAME="Ginkgo-Vikingo-Caf_rV0.1_Proton$(date '+%Y%m%d-%H%M').zip"
...
mkdir -p out
make O=out ARCH=arm64 ginkgo-vikingo-perf_defconfig

if [[ $1 == "-r" || $1 == "--regen" ]]; then
cp out/.config arch/arm64/configs/ginkgo-vikingo-perf_defconfig
